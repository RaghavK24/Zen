...
class Book(db.Model):
    ...
    def update_status(self):
        '''Update status to available if return date of issue is exceeded'''
        if self.status == "reserved":
            issue = Issue.query.filter_by(book_id = self.id, status="issued")
            if issue is None:
...
class Issue(db.Model):
    ...
    def update_status(self):
        '''Update status to revoked if return date is exceeded'''
        if self.status == "issued":
            if datetime.now() > (self.issue_date + timedelta(days=7)):
                self.status = "revoked"
...
def create_initial_data(db):
    ...
    base_created_date = base_date
    base_updated_date = base_date + timedelta(days=7)  # One week later for updated date

    # Get the current date
    current_date = datetime.now()

    # Initialize dictionaries for created and updated timestamps
    created_timestamp = {}
    updated_timestamp = {}

    # Assign creation and update dates for section names and IDs
    for section_type, section_id in zip(section_types, section_ids):
        # Assign creation and update dates for section names and IDs
        created_date = min(base_created_date + timedelta(days=section_types.index(section_type) * 2), current_date)
        updated_date = min(base_updated_date + timedelta(days=section_types.index(section_type) * 2), current_date)
        created_timestamp[section_type] = created_date
        updated_timestamp[section_type] = updated_date
        created_timestamp[section_id] = created_date
        updated_timestamp[section_id] = updated_date
    ...
    for section_type in section_types:
        section = Section(
            ...
            created_timestamp=created_timestamp[section_type],
            updated_timestamp=updated_timestamp[section_type],
        )
    ...
    for book_data in book_samples:
        book = Book(
            ...
            created_timestamp=created_timestamp[book_data["section_id"]],
            updated_timestamp=updated_timestamp[book_data["section_id"]],
        )
        books.append(book)
