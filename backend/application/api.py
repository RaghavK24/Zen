...
class MaxRequests(HTTPException):
    def __init__(self, message):
        self.response = make_response(jsonify(message), 405)
...
class SectionAPI(Resource):
    ...
    def post(self):
        ...
        if user.role == 'admin':
            ...
            raise AuthorizationError(message="only admin can make sections")
...
book_request_parse.add_argument("price", type=int)
...
class RequestAPI(Resource):
    ...
    def post(self):
        ...
        if user_id is None:
            ...
            issue_count = Issue.query.filter_by(user_id = user_id,status="approved").count()
            request_count = Request.query.filter_by(user_id=user_id).count()
            if (issue_count + request_count) == 5:
                raise MaxRequests("You have more than 5 issues/requests")
            user=User.query.filter_by(id=user_id).first()
            print(user)
            if user is None:
                raise NotFound("user not found")
...
issue_parse = reqparse.RequestParser(bundle_errors=True)
issue_parse.add_argument("user_id", type=int, required=True)
issue_parse.add_argument("book_id", type=int, required=True)
issue_parse.add_argument("issue_date", type=datetime)
...
class IssueAPI(Resource):
    ...
    def get(self, id=None):
        ...
        if id is None:
            ...
            issue = Issue.query.filter_by(user_id=user_id, id=id).first()
            if issue is None:
                raise NotFound("issue not found")
            else:
                return make_response(issue.to_dict(), 200)
    ...
    def get(self, status):
        ...
        if status in [1,2]:
            if status==1:
                issues = Issue.query.filter_by(status="issued")
            else:
                issues = Issue.query.filter_by(status="revoked")
            data = [issue.to_dict() for issue in issues]

            return make_response(data, 200)
        else:
            raise BadRequest("status not issued or revoked")
    ...
    def post(self):
        ...
        args = issue_parse.parse_args()
        user_id = args.get("user_id", None)
        if user_id is None:
            raise BadRequest("user_id not provided")
        book_id = args.get("book_id", None)
        if book_id is None:
            raise BadRequest("book_id not provided")
        book = Book.query.filter_by(id=book_id).first()
        ...
        if issue_date is None:
            ...
            if issue_date > datetime.now():
                raise BadRequest("issue_date is in future")
        issue = Issue.query.filter_by(user_id=user_id,book_id=book_id,status="issued").first()
        if issue is not None:
            print("=== issue already exists === ")
            raise BadRequest("issue already exists")
        ...
        book.status = "reserved"
        if issue is None:
            raise InternalError(message="error creating issue")
        try:
        ...
        return "Issue made successfully", 200
    ...
    def put(self, user_id, book_id):
        ...
        issue = Issue.query.filter_by(user_id=user_id,book_id=book_id,status="issued").first()
        if issue is None:
            print("=== no current issues available === ")
            raise BadRequest("no current issue exist")
        ...
        try:
            ...
            cache.clear()
                ...
                raise InternalError(message="error updating issue")

        return "Issue updated successfully", 200
    ...
    def delete(self,id):
        ...
        try:
            ...
            db.session.rollback()
            raise InternalError(message="Error in deleting")
        return "Issue deleted successfully", 200
