...
from application.models import User,Book, Section, Request,Issue,Order
...
def admin():
    ...
    data["request_total"] = Request.query.count()
    ...
    data["issues_total"] = Issue.query.count()
    ...
    for i in range(7):
        ...
        count = Request.query.filter(func.date(Request.created_timestamp) == date.date()).count()
        request_data.append({"date":date.date().strftime('%d/%b'), "count": count})
...
# @routes.route("/librarian", methods=["GET"])
# @jwt_required()
# @access(["librarian"])
# @cache.memoize(timeout=50)
# def admin():
...
#     data["section"] = Section.query.count()
#     data["books"] = Book.query.count()
#     data["requests"] = Request.query.count()
...
#         date = datetime.now() - timedelta(days=i)
#         count = Request.query.filter(func.date(Request.created_timestamp) == date.date()).count()
#         requests_data.append({"date":date.date().strftime('%d/%b'), "count": count})
#     data["requests_week"] = requests_data
...
#     # data["revenue_today"] = 0
#     data["requests_today"] = Request.query.filter(func.date(Request.created_timestamp) == datetime.today().date()).count()
